...
physical_constants = {
    'lightvel': ['velocity of light in vacuum', 'c', cloc, 0.0, 0.0, '299 792 458(ex) m/s', 'CODATA 2006'],
    ...
    'magn-const': ['magnetic permeability of vacuum', 'mu_0', 4e-7 * math.pi, 0.0, 0.0, '1.256 637 061... e-6 N A^-2', ''],
    'elec-const': ['dielectric permittivity of vacuum', 'eps_0', 1e7 / (4 * math.pi * cloc * cloc), 0.0, 0.0, '8.854 187 817... e-12 F/m', ''],
    ...
    'magnmom-e': ['electron magnetic moment', 'mu_e', -9.28476377e-24, 2.3e-31, 2.5
...
alpha = physical_constants['alpha'][2]
a_0 = physical_constants['bohrradius'][2]
c = cloc
e = physical_constants['charge-e'][2]
eps_0 = physical_constants['elec-const'][2]
F = physical_constants['faraday'][2]
G = physical_constants['gravit'][2]
g_e = physical_constants['gfactor-e'][2]
g_p = physical_constants['gfactor-p'][2]
gamma_p = physical_constants['gyromagratio-p'][2]
h = physical_constants['planck'][2]
hbar = physical_constants['dirac'][2]
k_B = physical_constants['boltzmann'][2]
m_d = physical_constants['mass-d'][2]
m_e = physical_constants['mass-e'][2]
m_n = physical_constants['mass-n'][2]
m_p = physical_constants['mass-p'][2]
mu_B = physical_constants['bohrmagn'][2]
mu_e = physical_constants['magnmom-e'][2]
mu_N = physical_constants['nuclmagn'][2]
mu_p = physical_constants['magnmom-p'][2]
mu_0 = physical_constants['magn-const'][2]
N_A = physical_constants['avogadro'][2]
R = physical_constants['gas'][2]
sigma = physical_constants['stefan-boltzm'][2]
u = physical_constants['amu'][2]
...
def help():
    ...
    for key in sorted(physical_constants.keys()):
        print(key)
...
def sd(key):
    return physical_constants[key][3]

def relsd(key):
    return physical_constants[key][4]

def descr(key):
    print(f'Description of {key}:')
    print(f'  Name:               {physical_constants[key][0]}')
    print(f'  Symbol (if avail.): {physical_constants[key][1]}')
    print(f'  Value:              {physical_constants[key][2]}')
    print(f'  Standard deviation: {physical_constants[key][3]}')
    print(f'  Relative stdev:     {physical_constants[key][4]}')
    print(f'  value(sd) unit:     {physical_constants[key][5]}')
    print(f'  Source:             {physical_constants[key][6]}\n')
